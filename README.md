...
接著，你可以自行修改 `config.json` 裡的「起點」與「終點」座標：
...
3. 回到原本的視窗執行 `python3 spoofy.py` 或 `python3 gui_spoofy.py`。

---

## 🌟 功能簡介

1. **常用起點 -> 常用終點 (行走模擬)**：一鍵開始自動導航（座標於 `config.json` 設定）。
2. **搜尋地點名稱**：(僅限 CLI) 直接輸入關鍵字搜尋全球地點。
3. **手動輸入座標**：直接貼上 Google Maps 複製的座標數字即可瞬間移動。
4. **自訂導航移動**：手動輸入起點、終點與時速進行模擬。

## 🔒 關於定位鎖定

- 執行定位後，程式會自動進入「鎖定模式」防止手機自動跳回真實位置。
- **[CLI]** 若要解除鎖定並回到主選單，只需在終端機按 **[Enter]** 鍵。
- **[CLI]** 導航行走中，可隨時按下 **[Ctrl + S]** 中斷。
