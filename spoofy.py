...
class CLISpoofer:
    def __init__(self, provider, is_ios17):
        ...
        self.core = SpooferCore(
            provider, is_ios17, log_callback=print, progress_callback=progress_cb
        )
    ...
    async def walk(self, start_coords, end_coords, speed_kmh=5.0, use_real_route=True):
        ...
        walk_task = asyncio.create_task(
            self.core.walk(start_coords, end_coords, speed_kmh, use_real_route)
        )
        try:
...
def load_config():
    ...
    start_coords = (
        start_data.get("coords") if isinstance(start_data, dict) else start_data
    )
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                use_real = input("請選擇 (預設為 1): ").strip() != "2"
                await spoofer.walk(
                    tuple(s), tuple(e), speed_kmh=sp, use_real_route=use_real
                )
            elif choice == "q":
                break
